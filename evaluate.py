...
def evaluate():
    ...
    with torch.no_grad():
        for images, labels in val_loader:
            ...
            logits = model(images)
            prob_map = torch.softmax(logits, dim=1)[:, 1]
            preds = (prob_map > 0.5).cpu().numpy().astype(float)
            probs = prob_map.cpu().numpy()

            patient_probs = probs.reshape(probs.shape[0], -1).max(axis=1)
            all_probs.append(patient_probs)
            all_preds.append(preds)
            all_targets.append(labels[:, 0].numpy())
